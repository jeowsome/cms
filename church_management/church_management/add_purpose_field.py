...
def run():
    ...
    if not any(f.fieldname == "purpose" for f in doc.fields):
        # Add purpose field
        # Place it after 'amount' or 'worker'? 'source' is there.
        # Let's put it after 'remarks' or before 'status'.
        # JSON order: status, worker, amount, received_by, received_date, source, remarks

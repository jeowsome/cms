...
async function save() {
  ...
  try {
    const saved = await call("church_management.api.donation.save_donation", {
      doc: { name: props.name, donated_amounts: items },
    });
...
<template>
  <div class="px-4 sm:px-6 py-4 sm:py-6 max-w-5xl mx-auto w-full">
    <PageHeader
      ...
      :subtitle="doc?.assigned_to ? `Recorded by ${doc.assigned_to}` : 'Donation record'"
    >
      <template #actions>
        <AppButton
      ...
      <div class="bg-white rounded-2xl border border-gray-200 overflow-hidden">
        ...
        >
          ...
          <select
            ...
            :title="(purposes || []).length ? (isMissionRow(row) ? 'Mission entries need admin approval before they post to the ledger' : '') : 'No purposes defined yet — add them via Manage purposes below'"
            @change="markDirty"
          >
